"""
CIS Audit Check: /etc/group Permissions (6.1.3)

Ensures that /etc/group is owned by root and has appropriate permissions.
...
class GroupPermissionsCheck(BaseCheck):
    """Check /etc/group file permissions."""

    id = "perm_group"
    name = "/etc/group Permissions"
    description = (
        "Verifies that /etc/group is owned by root:root "
    ...
    def run(self) -> CheckResult:
        ...
        if not check_result["exists"]:
            return CheckResult.failed_result(
                ...
                remediation=f"Restore {self.FILE_PATH} from backup or reinstall coreutils",
        ...
        if issues:
            return CheckResult.failed_result(
                ...
                remediation=(
                    ...
                    f"CIS Benchmark: 6.1.3 - Ensure permissions on /etc/group are configured"
                ),
                severity=self.severity,
                requires_root=self.requires_root,
                details=check_result,
