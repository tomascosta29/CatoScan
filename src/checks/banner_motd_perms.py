"""
CIS Audit Check: /etc/motd Permissions (1.6.4)

Ensures that /etc/motd is owned by root and has appropriate permissions.
...
class BannerMotdPermsCheck(BaseCheck):
    """Check /etc/motd file permissions."""

    id = "banner_motd_perms"
    name = "/etc/motd Permissions"
    description = (
        "Verifies that /etc/motd is owned by root:root "
    ...
    def run(self) -> CheckResult:
        """Execute the /etc/motd permissions check.
        ...
        if issues:
            return CheckResult.failed_result(
                ...
                remediation=(
                    ...
                    f"CIS Benchmark: 1.6.4 - Ensure permissions on /etc/motd are configured"
                ),
                severity=self.severity,
                requires_root=self.requires_root,
                details=check_result,
