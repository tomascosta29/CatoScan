"""
CIS Audit Check: /var Partition

Checks if /var is mounted as a separate partition (CIS 1.1.6).
"""
...
class VarPartitionCheck(BaseCheck):
    """Check if /var is mounted as a separate partition."""

    id = "fs_partition_var"
    name = "/var Partition"
    description = (
        "Verifies that /var is mounted as a separate partition "
        "to prevent system services from filling the root filesystem"
    )
    severity = Severity.HIGH
    requires_root = True

    def _get_mount_info(self) -> dict:
        """Get mount information for /var.
        ...
        try:
            ...
            if result.returncode == 0:
                for line in result.stdout.splitlines():
                    if " on /var " in line and " on /var/" not in line:
                        info["mounted"] = True
    ...
    def run(self) -> CheckResult:
        """Execute the /var partition check.
        ...
        if mount_info["mounted"]:
            return CheckResult.passed_result(
                ...
                message=(
                    f"/var is mounted as a separate partition "
        ...
        if mount_info["fstab_entry"]:
            return CheckResult.failed_result(
                check_id=self.id,
                check_name=self.name,
                message="/var is defined in /etc/fstab but not currently mounted",
                remediation=(
                    "Mount /var filesystem:\n"
                    "1. Check fstab entry: grep /var /etc/fstab\n"
                    "2. Mount /var: sudo mount /var\n"
                    "3. Verify: mount | grep 'on /var '"                ),
                severity=self.severity,
                requires_root=self.requires_root,
                details=mount_info,
            )
        ...
        return CheckResult.failed_result(
            check_id=self.id,
            check_name=self.name,
            message="/var is not mounted as a separate partition",
            remediation=(
                "Configure /var as a separate partition:\n"
                "1. Create a dedicated partition for /var during installation or using LVM\n"
                "2. Add to /etc/fstab (example):\n"
                "   /dev/sdXn /var ext4 defaults,nodev,nosuid,noexec 0 2\n"
                "3. Mount /var: sudo mount /var\n"
                "4. Verify: mount | grep 'on /var '\n\n"
                "Note: This requires planning during system installation or maintenance mode.\n"
                "      Separating /var prevents log files from filling the root filesystem."
            ),
            severity=self.severity,
            requires_root=self.requires_root,
            details=mount_info,
