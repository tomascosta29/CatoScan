"""
CIS Audit Check: /etc/shells Permissions (6.1.9)

Ensures that /etc/shells is owned by root and has appropriate permissions.
...
class ShellsPermissionsCheck(BaseCheck):
    """Check /etc/shells file permissions."""

    id = "perm_shells"
    name = "/etc/shells Permissions"
    description = (
        "Verifies that /etc/shells is owned by root:root "
    ...
    def run(self) -> CheckResult:
        """Execute the /etc/shells permissions check.
        ...
        if not check_result["exists"]:
            return CheckResult.failed_result(
                ...
                remediation=f"Restore {self.FILE_PATH} from backup or reinstall setup package",
        ...
        if issues:
            return CheckResult.failed_result(
                ...
                remediation=(
                    ...
                    f"CIS Benchmark: 6.1.9 - Ensure permissions on /etc/shells are configured"
                ),
                severity=self.severity,
                requires_root=self.requires_root,
                details=check_result,
