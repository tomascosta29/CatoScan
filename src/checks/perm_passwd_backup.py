"""
CIS Audit Check: /etc/passwd- Backup Permissions (6.1.2)

Ensures that /etc/passwd- backup file is owned by root and has appropriate permissions.
...
class PasswdBackupPermissionsCheck(BaseCheck):
    """Check /etc/passwd- backup file permissions."""

    id = "perm_passwd_backup"
    name = "/etc/passwd- Backup Permissions"
    description = (
        "Verifies that /etc/passwd- backup file is owned by root:root "
    ...
    def run(self) -> CheckResult:
        """Execute the /etc/passwd- permissions check.
        ...
        if not check_result["exists"]:
            # Backup file may not exist - this is not necessarily a failure
            # but we should report it
            return CheckResult.passed_result(
                check_id=self.id,
                check_name=self.name,
                message=f"{self.FILE_PATH} does not exist (backup file may not have been created yet)",
        ...
        if issues:
            return CheckResult.failed_result(
                ...
                remediation=(
                    ...
                    f"CIS Benchmark: 6.1.2 - Ensure permissions on /etc/passwd- are configured"
                ),
                severity=self.severity,
                requires_root=self.requires_root,
                details=check_result,
