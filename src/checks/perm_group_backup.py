"""
CIS Audit Check: /etc/group- Backup Permissions (6.1.4)

Ensures that /etc/group- backup file is owned by root and has appropriate permissions.
...
class GroupBackupPermissionsCheck(BaseCheck):
    """Check /etc/group- backup file permissions."""

    id = "perm_group_backup"
    name = "/etc/group- Backup Permissions"
    description = (
        "Verifies that /etc/group- backup file is owned by root:root "
    ...
    def run(self) -> CheckResult:
        """Execute the /etc/group- permissions check.
        ...
        if issues:
            return CheckResult.failed_result(
                ...
                remediation=(
                    ...
                    f"CIS Benchmark: 6.1.4 - Ensure permissions on /etc/group- are configured"
                ),
                severity=self.severity,
                requires_root=self.requires_root,
                details=check_result,
