"""
CIS Audit Check: /etc/cron.hourly Permissions (4.1.3)

Ensures that /etc/cron.hourly is owned by root:root and has permissions 700.
"""
...
class CronHourlyPermsCheck(BaseCheck):
    """Check /etc/cron.hourly directory permissions."""

    id = "cron_hourly_perms"
    name = "/etc/cron.hourly Permissions"
    description = (
        "Verifies that /etc/cron.hourly is owned by root:root "
    ...
    def run(self) -> CheckResult:
        """Execute the /etc/cron.hourly permissions check.
        ...
        if issues:
            return CheckResult.failed_result(
                ...
                remediation=(
                    ...
                    f"CIS Benchmark: 4.1.3 - Ensure permissions on /etc/cron.hourly are configured"
                ),
                severity=self.severity,
                requires_root=self.requires_root,
                details=check_result,
