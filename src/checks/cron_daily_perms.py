"""
CIS Audit Check: /etc/cron.daily Permissions (4.1.4)

Ensures that /etc/cron.daily is owned by root:root and has permissions 700.
"""
...
class CronDailyPermsCheck(BaseCheck):
    """Check /etc/cron.daily directory permissions."""

    id = "cron_daily_perms"
    name = "/etc/cron.daily Permissions"
    description = (
        "Verifies that /etc/cron.daily is owned by root:root "
    ...
    def run(self) -> CheckResult:
        """Execute the /etc/cron.daily permissions check.
        ...
        if issues:
            return CheckResult.failed_result(
                ...
                remediation=(
                    ...
                    f"CIS Benchmark: 4.1.4 - Ensure permissions on /etc/cron.daily are configured"
                ),
                severity=self.severity,
                requires_root=self.requires_root,
                details=check_result,
