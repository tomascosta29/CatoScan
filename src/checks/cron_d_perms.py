"""
CIS Audit Check: /etc/cron.d Permissions (4.1.7)

Ensures that /etc/cron.d is owned by root:root and has permissions 700.
"""
...
class CronDPermsCheck(BaseCheck):
    """Check /etc/cron.d directory permissions."""

    id = "cron_d_perms"
    name = "/etc/cron.d Permissions"
    description = (
        "Verifies that /etc/cron.d is owned by root:root "
    ...
    def run(self) -> CheckResult:
        """Execute the /etc/cron.d permissions check.
        ...
        if issues:
            return CheckResult.failed_result(
                ...
                remediation=(
                    ...
                    f"CIS Benchmark: 4.1.7 - Ensure permissions on /etc/cron.d are configured"
                ),
                severity=self.severity,
                requires_root=self.requires_root,
                details=check_result,
