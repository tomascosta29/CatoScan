"""
CIS Audit Check: /etc/issue Warning Banner (1.6.2)

Ensures that /etc/issue is configured with a proper warning banner
...
class BannerIssueCheck(BaseCheck):
    """Check /etc/issue warning banner configuration."""

    id = "banner_issue"
    name = "/etc/issue Warning Banner"
    description = (
        "Verifies that /etc/issue is configured with a proper warning banner "
    ...
    def run(self) -> CheckResult:
        """Execute the /etc/issue banner check.
        ...
        if check_result["contains_os_info"]:
            return CheckResult.failed_result(
                ...
                remediation=(
                    ...
                    f"CIS Benchmark: 1.6.2 - Ensure local login warning banner is configured properly"
                ),
                severity=self.severity,
                requires_root=self.requires_root,
                details=check_result,
