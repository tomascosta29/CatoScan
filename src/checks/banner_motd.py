"""
CIS Audit Check: /etc/motd Warning Banner (1.6.1)

Ensures that /etc/motd is configured with a proper warning banner
...
class BannerMotdCheck(BaseCheck):
    """Check /etc/motd warning banner configuration."""

    id = "banner_motd"
    name = "/etc/motd Warning Banner"
    description = (
        "Verifies that /etc/motd is configured with a proper warning banner "
    ...
    def run(self) -> CheckResult:
        """Execute the /etc/motd banner check.
        ...
        if check_result["contains_os_info"]:
            return CheckResult.failed_result(
                ...
                remediation=(
                    ...
                    f"CIS Benchmark: 1.6.1 - Ensure message of the day is configured properly"
                ),
                severity=self.severity,
                requires_root=self.requires_root,
                details=check_result,
