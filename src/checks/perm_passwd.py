"""
CIS Audit Check: /etc/passwd Permissions (6.1.1)

Ensures that /etc/passwd is owned by root and has appropriate permissions.
...
class PasswdPermissionsCheck(BaseCheck):
    ...
    description = (
        "Verifies that /etc/passwd is owned by root:root "
    ...
    def run(self) -> CheckResult:
        ...
        if not check_result["exists"]:
            return CheckResult.failed_result(
                ...
                remediation=f"Restore {self.FILE_PATH} from backup or reinstall passwd package",
        ...
        if issues:
            return CheckResult.failed_result(
                ...
                remediation=(
                    ...
                    f"CIS Benchmark: 6.1.1 - Ensure permissions on /etc/passwd are configured"
                ),
                severity=self.severity,
                requires_root=self.requires_root,
                details=check_result,
