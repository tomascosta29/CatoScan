"""
CIS Audit Check: /etc/issue.net Permissions (1.6.6)

Ensures that /etc/issue.net is owned by root and has appropriate permissions.
CIS recommends 644 (rw-r--r--) or more restrictive.
"""
...
class BannerIssueNetPermsCheck(BaseCheck):
    """Check /etc/issue.net file permissions."""

    id = "banner_issue_net_perms"
    name = "/etc/issue.net Permissions"
    description = (
        "Verifies that /etc/issue.net is owned by root:root "
        "and has permissions 644 or more restrictive"
    )
    ...
    def _check_permissions(self) -> dict:
        ...
        try:
            ...
            # Check mode (should be 644 or more restrictive)
            # More restrictive means fewer permissions for group/other
    ...
    def run(self) -> CheckResult:
        """Execute the /etc/issue.net permissions check.
        ...
        if issues:
            return CheckResult.failed_result(
                ...
                remediation=(
                    ...
                    f"  sudo chmod 644 {self.FILE_PATH}\n\n"
                    f"CIS Benchmark: 1.6.6 - Ensure permissions on /etc/issue.net are configured"
                ),
                severity=self.severity,
                requires_root=self.requires_root,
                details=check_result,
