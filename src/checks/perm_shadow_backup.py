"""
CIS Audit Check: /etc/shadow- Backup Permissions (6.1.6)

Ensures that /etc/shadow- backup file is owned by root and has appropriate permissions.
...
class ShadowBackupPermissionsCheck(BaseCheck):
    """Check /etc/shadow- backup file permissions."""

    id = "perm_shadow_backup"
    name = "/etc/shadow- Backup Permissions"
    description = (
        "Verifies that /etc/shadow- backup file is owned by root:root "
        ...
        if issues:
            return CheckResult.failed_result(
                ...
                remediation=(
                    ...
                    f"CIS Benchmark: 6.1.6 - Ensure permissions on /etc/shadow- are configured"
                ),
                severity=self.severity,
                requires_root=self.requires_root,
                details=check_result,
