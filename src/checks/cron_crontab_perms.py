"""
CIS Audit Check: /etc/crontab Permissions (4.1.2)

Ensures that /etc/crontab is owned by root:root and has permissions 600.
"""
...
class CronCrontabPermsCheck(BaseCheck):
    """Check /etc/crontab file permissions."""

    id = "cron_crontab_perms"
    name = "/etc/crontab Permissions"
    description = (
        "Verifies that /etc/crontab is owned by root:root "
    ...
    def run(self) -> CheckResult:
        """Execute the /etc/crontab permissions check.
        ...
        if issues:
            return CheckResult.failed_result(
                ...
                remediation=(
                    ...
                    f"CIS Benchmark: 4.1.2 - Ensure permissions on /etc/crontab are configured"
                ),
                severity=self.severity,
                requires_root=self.requires_root,
                details=check_result,
