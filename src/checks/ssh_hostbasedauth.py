"""
CIS Audit Check: SSH HostbasedAuthentication (5.1.9)

Ensures host-based authentication is disabled to prevent
...
class SSHHostbasedAuthCheck(BaseCheck):
    """Check SSH HostbasedAuthentication configuration."""

    id = "ssh_hostbasedauth"
    name = "SSH HostbasedAuthentication"
    description = (
        "Verifies that host-based authentication is disabled "
    ...
    def _parse_sshd_config(self) -> dict:
        """Parse SSH daemon configuration for HostbasedAuthentication setting.
    ...
    def run(self) -> CheckResult:
        """Execute the SSH HostbasedAuthentication check.
        ...
        if not config["files_read"]:
            # Default HostbasedAuthentication is no
            return CheckResult.passed_result(
                check_id=self.id,
                check_name=self.name,
                message=(
                    "HostbasedAuthentication not explicitly set. Default is 'no', "
                    "which is secure."
                ),
        ...
        if config["hostbasedauth"] is None:
            ...
            return CheckResult.passed_result(
                ...
                check_name=self.name,
                message=(
                    "HostbasedAuthentication directive not found. Default is 'no', "
                    "which is secure."
                ),
                severity=self.severity,
                requires_root=self.requires_root,
        ...
        if hostbasedauth == "no":
            return CheckResult.passed_result(
                check_id=self.id,
                check_name=self.name,
                message="HostbasedAuthentication is disabled - secure configuration",
                severity=self.severity,
                requires_root=self.requires_root,
                details=config,
            )
        ...
        if hostbasedauth == "yes":
            return CheckResult.failed_result(
                check_id=self.id,
                check_name=self.name,
                message="HostbasedAuthentication is enabled - security risk",
                remediation=(
