"""
CIS Audit Check: /etc/cron.monthly Permissions (4.1.6)

Ensures that /etc/cron.monthly is owned by root:root and has permissions 700.
"""
...
class CronMonthlyPermsCheck(BaseCheck):
    """Check /etc/cron.monthly directory permissions."""

    id = "cron_monthly_perms"
    name = "/etc/cron.monthly Permissions"
    description = (
        "Verifies that /etc/cron.monthly is owned by root:root "
    ...
    def run(self) -> CheckResult:
        """Execute the /etc/cron.monthly permissions check.
        ...
        if issues:
            return CheckResult.failed_result(
                ...
                remediation=(
                    ...
                    f"CIS Benchmark: 4.1.6 - Ensure permissions on /etc/cron.monthly are configured"
                ),
                severity=self.severity,
                requires_root=self.requires_root,
                details=check_result,
