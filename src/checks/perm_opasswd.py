"""
CIS Audit Check: /etc/opasswd Permissions (6.1.10)

Ensures that /etc/opasswd (old password storage for pam_cracklib/pam_pwquality)
is owned by root and has appropriate permissions.
CIS recommends 600 (rw-------) or more restrictive.
"""
...
class OpasswdPermissionsCheck(BaseCheck):
    """Check /etc/opasswd file permissions."""

    id = "perm_opasswd"
    name = "/etc/opasswd Permissions"
    description = (
        "Verifies that /etc/opasswd is owned by root:root "
        "and has permissions 600 or more restrictive"
    )
    severity = Severity.MEDIUM
    requires_root = True

    FILE_PATH = "/etc/opasswd"
    EXPECTED_MODE = 0o600
    ...
    def run(self) -> CheckResult:
        """Execute the /etc/opasswd permissions check.
        ...
        if not check_result["exists"]:
            # File may not exist if pam_pwquality with remember option is not configured
            return CheckResult.passed_result(
                check_id=self.id,
                check_name=self.name,
                message=f"{self.FILE_PATH} does not exist (old password file may not be in use)",
        ...
        if issues:
            return CheckResult.failed_result(
                ...
                remediation=(
                    ...
                    f"CIS Benchmark: 6.1.10 - Ensure permissions on /etc/opasswd are configured"
                ),
                severity=self.severity,
                requires_root=self.requires_root,
                details=check_result,
