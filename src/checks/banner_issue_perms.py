"""
CIS Audit Check: /etc/issue Permissions (1.6.5)

Ensures that /etc/issue is owned by root and has appropriate permissions.
...
class BannerIssuePermsCheck(BaseCheck):
    """Check /etc/issue file permissions."""

    id = "banner_issue_perms"
    name = "/etc/issue Permissions"
    description = (
        "Verifies that /etc/issue is owned by root:root "
    ...
    def run(self) -> CheckResult:
        """Execute the /etc/issue permissions check.
        ...
        if issues:
            return CheckResult.failed_result(
                ...
                remediation=(
                    ...
                    f"CIS Benchmark: 1.6.5 - Ensure permissions on /etc/issue are configured"
                ),
                severity=self.severity,
                requires_root=self.requires_root,
                details=check_result,
