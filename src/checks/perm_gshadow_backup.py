"""
CIS Audit Check: /etc/gshadow- Backup Permissions (6.1.8)

Ensures that /etc/gshadow- backup file is owned by root and has appropriate permissions.
...
class GshadowBackupPermissionsCheck(BaseCheck):
    """Check /etc/gshadow- backup file permissions."""

    id = "perm_gshadow_backup"
    name = "/etc/gshadow- Backup Permissions"
    description = (
        "Verifies that /etc/gshadow- backup file is owned by root:root "
        ...
        if issues:
            return CheckResult.failed_result(
                ...
                remediation=(
                    ...
                    f"CIS Benchmark: 6.1.8 - Ensure permissions on /etc/gshadow- are configured"
                ),
                severity=self.severity,
                requires_root=self.requires_root,
                details=check_result,
